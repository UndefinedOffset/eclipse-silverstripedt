...
public class SSTemplatesContentAssistPreferencesPage extends AbstractPreferencePage implements
    ...
    private static final String CONTENT_TYPE_ID = "ca.edchipman.silverstripepdt.SilverStripeTemplateSource"; //$NON-NLS-1$
    ...
    private void createContentsForCyclingGroup(Composite parent) {
        ICompletionProposalCategoriesConfigurationWriter configurationWriter = CompletionProposoalCatigoriesConfigurationRegistry.getDefault().getWritableConfiguration(CONTENT_TYPE_ID);
        
        if(configurationWriter != null) {
            fConfigurationBlock = new CodeAssistCyclingConfigurationBlock(CONTENT_TYPE_ID, configurationWriter);
